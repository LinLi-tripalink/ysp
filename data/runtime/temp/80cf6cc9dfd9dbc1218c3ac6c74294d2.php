<?php /*a:2:{s:83:"/data/wwwroot/bjwz.yyyybbbb.com/themes/admin_simpleboot3/admin/video/passindex.html";i:1668570290;s:75:"/data/wwwroot/bjwz.yyyybbbb.com/themes/admin_simpleboot3/public/header.html";i:1668570290;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
	<div class="wrap js-check-wrap">
		<ul class="nav nav-tabs">
			...
			<!-- <li><a href="<?php echo url('Video/add'); ?>">视频添加</a></li> -->
		</ul>
		
		<form class="well form-inline margin-top-20" method="post" action="<?php echo url('Video/passindex'); ?>">
			...
			<select class="select_2 form-control" name="ordertype">
				...
				<option value="3" <?php if(input('request.ordertype') == 3): ?>selected<?php endif; ?> >分享数排序</option>
			...
			<select class="select_2 form-control" name="classid">
				...
				<?php if(is_array($classify) || $classify instanceof \think\Collection || $classify instanceof \think\Paginator): $i = 0; $__LIST__ = $classify;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
					<option value="<?php echo $vo['id']; ?>" <?php if(input('request.classid') == $vo['id']): ?>selected<?php endif; ?> ><?php echo $vo['title']; ?></option>
				<?php endforeach; endif; else: echo "" ;endif; ?>		
			</select>

			关键字： 
			<input class="form-control" type="text" name="keyword" style="width: 200px;" value="<?php echo input('request.keyword'); ?>" placeholder="视频ID/会员ID">
			<input class="form-control" type="text" name="keyword1" style="width: 200px;" value="<?php echo input('request.keyword1'); ?>" placeholder="视频标题">
			...
			<a class="btn btn-danger" href="<?php echo url('Admin/Video/passindex'); ?>">清空</a>
		</form>		
		
		<form method="post" class="js-ajax-form">

			<div class="table-actions">
                <button class="btn btn-danger btn-sm js-ajax-submit" type="submit" data-action="<?php echo url('Video/setBatchDel'); ?>"
                        data-subcheck="true">批量删除
                </button>

                <button class="btn btn-danger btn-sm js-ajax-submit" type="submit" data-action="<?php echo url('Video/setBatchXiajia',array('status'=>'1')); ?>"
			...
			<table class="table table-hover table-bordered">
				...
				<tbody>
					<?php $isdel=array("0"=>"上架","1"=>"下架");$status=array("0"=>"待审核","1"=>"通过","2"=>"不通过"); if(is_array($lists) || $lists instanceof \think\Collection || $lists instanceof \think\Paginator): if( count($lists)==0 ) : echo "" ;else: foreach($lists as $key=>$vo): ?>
					<tr>
						<td>
						...
						<td><img src="<?php echo get_upload_path($vo['thumb']); ?>" class="imgtip" /></td>
						...
						<td><?php if($vo['isgoods'] == '1'): ?>是<?php else: ?>否<?php endif; ?></td>
						<td><?php echo date('Y-m-d H:i:s',$vo['addtime']); ?></td>
						<td align="center">
							<a class="btn btn-xs btn-primary" href="javascript:void(0)" onclick="videoListen(<?php echo $vo['id']; ?>)" >观看</a>
							
							<a class="btn btn-xs btn-primary" href="<?php echo url('Video/edit',array('id'=>$vo['id'],'from'=>'passindex')); ?>" >编辑</a>
							 <?php if($vo['isdel'] == '0'): ?>
							<a class="btn btn-xs btn-danger"  href="javascript:void (0)" onclick="xiajia(<?php echo $vo['id']; ?>)" >下架</a><?php endif; ?>
							...
							<!-- <a href="<?php echo url('Video/del',array('id'=>$vo['id'])); ?>" class="js-ajax-dialog-btn" data-msg="您确定要删除吗？">删除</a> -->
						</td>
					</tr>
					<?php endforeach; endif; else: echo "" ;endif; ?>
				</tbody>
	...
	<script src="/static/js/admin.js?t=<?php echo $time; ?>"></script>
