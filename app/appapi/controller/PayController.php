...
class PayController extends HomebaseController {
	...
	public function notify_ali() {
		...
		if($verify_result) {//验证成功
			...
			if($_POST['trade_status'] == 'TRADE_FINISHED') {
				...
				$orderinfo=Db::name("user_charge")
					->where("orderno='{$out_trade_no}' and money='{$total_fee}' and status='0' and type='1'")
					->find();	
				...
				if($orderinfo){
					...
					/* 更新 订单状态 */
					Db::name("user_charge")->where("id='{$orderinfo['id']}'")->update(array("status"=>1,"trade_no"=>$trade_no));
	...
	private function orderServer(){
		...
		$orderinfo=Db::name("user_charge")->where("orderno='{$info['out_trade_no']}' and status='0' and type='2'")->find();
		...
		if($orderinfo){
			...
			Db::name("user")->where("id='{$orderinfo['touid']}'")->setInc("coin",$coin);
			/* 更新 订单状态 */
			Db::name("user_charge")->where("id='{$orderinfo['id']}'")->update(array("status"=>1,"trade_no"=>$info['transaction_id']));
		}else{
	...
	*/
	...
	public function notify_ios(){
		...
		$iforderinfo=Db::name("user_charge")->where("trade_no='{$info['transaction_id']}' and type='3'")->find();
		...
		//判断订单是否存在
		$orderinfo=Db::name("user_charge")->where("orderno='{$out_trade_no}' and status='0' and type='3'")->find();
		if($orderinfo){
			/* 更新会员虚拟币 */
			$coin=$orderinfo['coin']+$orderinfo['coin_give'];
			Db::name("user")->where("id='{$orderinfo['touid']}'")->setInc("coin",$coin);
	...
	public function logali($msg){
		file_put_contents(CMF_ROOT.'data/log/paylog/logali_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}		
	/* 打印log */
	public function logwx($msg){
		file_put_contents(CMF_ROOT.'data/log/paylog/logwx_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}			
	/* 打印log */
	public function logios($msg){
		file_put_contents(CMF_ROOT.'data/log/paylog/logios_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}					
