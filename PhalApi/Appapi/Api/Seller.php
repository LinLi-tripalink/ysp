...
class Api_Seller extends PhalApi_Api {
	...
	public function setGoods(){
        ...
        $thumbs_arr=explode(',', $thumbs);
        $thumbs_arr_format=[];
        $thumbs_arr_format_str='';
        foreach ($thumbs_arr as $k => $v) {
            $thumbs_arr_format[]=setCloudType($v);
        }

        $thumbs_arr_format_str=implode(',', $thumbs_arr_format);
        ...
        if($pictures){
            ...
            $pictures_format=[];
            $pictures_format_str='';
            foreach ($pictures_arr as $k => $v) {
                $pictures_format[]=setCloudType($v);
            }

            $pictures_format_str=implode(',',$pictures_format);
        }
		...
		$data=array(
			...
			'video_url'=>setCloudType($video_url),
			'video_thumb'=>setCloudType($video_thumb),
			'thumbs'=>$thumbs_arr_format_str,
			'content'=>$content,
			'pictures'=>$pictures_format_str,
			'specs'=>urldecode(json_encode($new_spec_arr)),
			'postage'=>$postage,
			'low_price'=>$low_price,
			'addtime'=>time(),
