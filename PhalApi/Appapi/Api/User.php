...
class Api_User extends PhalApi_Api {
	...
	public function getRules() {
		return array(
			...
			'setAdvertiser'=>array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
				'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
				'username' => array('name' => 'username', 'type' => 'string', 'require' => true, 'desc' => '认证主体名称'),
				'mobile' => array('name' => 'mobile', 'type' => 'string', 'require' => true, 'desc' => '联系方式'),
				'auth_desc' => array('name' => 'auth_desc', 'type' => 'string', 'require' => true, 'desc' => '认证说明'),
				'img1' => array('name' => 'img1', 'type' => 'string',  'desc' => '资质图片1'),
				'img2' => array('name' => 'img2', 'type' => 'string',  'desc' => '资质图片2'),
			),
			...
			'setTeenagers' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
				'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
				'type' => array('name' => 'type', 'type' => 'string', 'require' => true, 'desc' => '1设置密码并开启 2开启青少年模式 3关闭青少年模式 4修改密码'),
				'pwd' => array('name' => 'pwd', 'type' => 'string', 'require' => true, 'desc' => '密码'),
				'newpwd' => array('name' => 'newpwd', 'type' => 'string', 'default'=>'', 'desc' => '修改新密码'),
				'newspwd' => array('name' => 'newspwd', 'type' => 'string', 'default'=>'', 'desc' => '确认新密码'),

			),
			'reduceTeenagers' => array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
				'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
				'duration' => array('name' => 'duration', 'type' => 'int', 'require' => true, 'desc' => '使用时长(秒)'),
	 ...
	 * @return string info[0].bonus_switch 每日签到开关 0关 1开
	 * @return string msg 提示信息
	 */
	public function getBaseInfo(){
        ...
        $info['bonus_switch']=$configpri['bonus_switch'];       
	 ...
	 * @return array info[0].collectionVideos 收藏视频数
	 ...
	 * @return string info[0].userinfo 视频发布者信息
	 */
	public function getCollectionVideos(){
		...
		$domain = new Domain_User();
		$res=$domain->getCollectionVideos($uid,$touid,$p);
		if($res==1001){
			$rs['code']=0;
			$rs['msg']="暂无视频列表";
			return $rs;
	...
	*/
	...
	public function checkLiveVipStatus(){
		...
		$isad=$domain->isUserAdvertiser($uid);
		$result['isad']=(string)$isad;
		...
		if(!$result['video_status'] && $vip_switch){
	 ...
	 * @return string info[0].is_bonus 是否已签到 0 否 1 是
	...
	public function setAdvertiser(){
		$rs = array('code' => 0, 'msg' => '认证提交成功,请等待管理员审核', 'info' => array());
		
		$uid=checkNull($this->uid);
		$token=checkNull($this->token);
		$username=checkNull($this->username);
		$mobile=checkNull($this->mobile);
		$auth_desc=checkNull($this->auth_desc);
		$img1=checkNull($this->img1);
		$img2=checkNull($this->img2);
		...
		$mobile_check=checkMobile($mobile);
		if(!$mobile_check){
			$rs['code']=1004;
			$rs['msg']='手机号码错误';
			return $rs;
		...
		if($img1){
			$img1=setCloudType($img1);	
		}

		if($img2){
			$img2=setCloudType($img2);	
		}
		...
		$res=$domain->setAdvertiser($data);

		if($res==1001){
			$rs['code']=1007;
			$rs['msg']='广告主认证失败,请稍后重试';
			return $rs;
		}
	 ...
	 * @return string msg 提示信息
	 * @return array info 返回信息
	 * @return array info[0]['has_auth'] 是否有认证信息 0 否 1 是
	 * @return array info[0]['auth_info']['status'] 是否有认证信息 0 否 1 是
	 * */
	public function getAdvertiserInfo(){
		...
		$rs['info'][0]['auth_info']=$res;
		if(!$res){
			$status='0';
		}else{
			$status='1';
		}
		$rs['info'][0]['has_auth']=$status;
		return $rs;
	}
	 ...
	 * @retun string msg 返回信息
	 * @retun array info 返回信息
	 * @retun array info[0]['ispwd'] 是否设置密码 0否  1是
	 * @retun array info[0]['isstate'] 是否开启青少年模式 0否  1是
	 * @retun array info[0]['duration'] 定时请求秒数
	 * @retun array info[0]['useinfo'] 是否可以使用详情
	 * @retun array info[0]['useinfo']['isuser'] 使用状态 0可以  1已达上限  2非规定时间内使用
	 * @retun array info[0]['useinfo']['msg'] 提示话术
	 * @retun array info[0]['prompt'] 提示话术
	 * @retun array info[0]['lists'] 提示话术
	 * */
	public function checkTeenagers(){
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
	 ...
	 * @retun string msg 返回信息
	 * @retun array info 返回信息
	 * @retun array info[0]['isuser'] 青少年模式使用状态 0可以  1已达上限  2非规定时间内使用
	 * @retun array info[0]['isfirst'] 是否是第一次设置 0否 1是
	 * @retun array info[0]['msg'] 提示语
	 * */
	public function setTeenagers(){
		$rs = array('code' => 0, 'msg' => '操作成功', 'info' => array());
		
		$uid=checkNull($this->uid);
		$token=checkNull($this->token);
		$type=checkNull($this->type);
		$pwd=checkNull($this->pwd);
		$newpwd=checkNull($this->newpwd);
		$newspwd=checkNull($this->newspwd);
		...
		//1设置密码并开启 2开启青少年模式 3关闭青少年模式 4修改密码 5重新设置密码
		$isfirst=0;
		...
		$domain=new Domain_User();
		$res=$domain->setTeenagers($uid,$type,$pwd,$newpwd,$newspwd);
		if($res==1001){
            $rs['code'] = 1001;
			$rs['msg'] = '您已设置过青少年模式,请前往开启';
			return $rs;
        }else if($res==1002){
            $rs['code'] = 1002;
			$rs['msg'] = '您未设置过青少年模式,请前往设置';
			return $rs;
        }else if($res==1003){
            $rs['code'] = 1003;
			$rs['msg'] = '当前输入密码错误';
	 ...
	 * @retun array info[0]['msg'] 提示语
	 * */
	public function reduceTeenagers(){
		$rs = array('code' => 0, 'msg' => '操作成功', 'info' => array());
		...
		$domain=new Domain_User();
		$res=$domain->reduceTeenagers($uid,$duration);
		if($res==1001){
            $rs['code'] = 1001;
			$rs['msg'] = '您未开启过青少年模式,请前往开启';
			return $rs;
        }
