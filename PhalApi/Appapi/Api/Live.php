...
class Api_Live extends PhalApi_Api {
	...
	public function getRules() {
		return array(
			...
			'superStopRoom' => array(
                ...
                'liveuid' => array('name' => 'liveuid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '主播ID'),
                'banlong' => array('name' => 'banlong', 'type' => 'string',  'desc' => '封禁时间'),
			...
			'getLiveFollow'=>array(
            	'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '会员ID'),
            	'touid' => array('name' => 'touid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '对方会员ID'),
	 ...
	 * @return object info[0].liveban_score 直播封禁状态 1展示  0不展示
	 * @return object info[0].liveban_msg 直播封禁话
    ...
    public function getLiveParameters(){
		...
		//封禁话术
		$liveban=checkLiveBan($uid);
		
    	$rs['info'][0]['liveban_score']=$liveban['score'];
    	$rs['info'][0]['liveban_msg']=$liveban['msg'];
	...
	public function enterRoom() {
        ...
        if($pkuid){
            $pkinfo['pkuid']=$pkuid;
			$pkinfo['isattention']=(string)isAttention($uid,$pkuid); //是否关注
            $pkinfo['userinfo']=getUserInfo($pkuid);
	...
	public function getPop() {
		...
		//封禁时间
		$info['banlong']=array(
			'0'=>['id'=>'30_1','title'=>'30分钟'],
			'1'=>['id'=>'1_2','title'=>'1天'],
			'2'=>['id'=>'7_2','title'=>'7天'],
			'3'=>['id'=>'15_2','title'=>'15天'],
			'4'=>['id'=>'30_2','title'=>'30天'],
			'5'=>['id'=>'90_2','title'=>'90天'],
			'6'=>['id'=>'180_2','title'=>'180天'],
			'7'=>['id'=>'0_0','title'=>'永久'],
		);
	...
	public function superStopRoom(){
		...
		$result = $domain->superStopRoom($uid,$token,$liveuid,$type,$banlong);
    ...
    /**
	 * 主播关注信息
	 * @desc 用于主播pk时获取对方资料详情
	 * @return int code 状态码，0表示成功
	 * @return string msg 提示信息
	 * @return string info[0].isattention 是否关注主播，0表示未关注，1表示已关注
	 * @return array info 返回信息
	 */
	public function getLiveFollow(){
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
		$uid=checkNull($this->uid);
        $touid=checkNull($this->touid);
