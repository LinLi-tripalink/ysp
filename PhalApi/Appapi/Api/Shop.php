...
class Api_Shop extends PhalApi_Api {
	...
	public function getRules() {
		return array(
			...
			'batchDelCollect'=>array(
				'uid' => array('name' => 'uid', 'type' => 'int', 'min' => 1, 'require' => true, 'desc' => '用户ID'),
				'token' => array('name' => 'token', 'type' => 'string', 'require' => true, 'desc' => '用户token'),
				'goodsids' => array('name' => 'goodsids', 'type' => 'string','require' => true, 'desc' => '商品ID集合'),
			),

			'getVideoBindGoodsInfo'=>array(
				'goodsid' => array('name' => 'goodsid', 'type' => 'int', 'desc' => '商品ID'),
			),
			...
			'setShowGoods' => array(
                ...
                'token' => array('name' => 'token', 'type' => 'string', 'desc' => '用户token'),
                'goodsid' => array('name' => 'goodsid', 'type' => 'int', 'desc' => '商品ID'),
				'isshow' => array('name' => 'isshow', 'type' => 'int', 'desc' => '展示状态，0 否 1 是'),
	...
	*/
	...
	public function shopApply(){
		...
		$data=array(
	        ...
	        'certificate'=>setCloudType($certificate),
	        'other'=>setCloudType($other),
	        'status'=>1
	...
	public function batchDelCollect(){
		...
		$uid=checkNull($this->uid);
		$token=checkNull($this->token);
		$goodsids=checkNull($this->goodsids);
		...
		$new_goodsids_arr=[];
		$goodsids_arr=explode(',', $goodsids);
		foreach ($goodsids_arr as $k => $v) {
			if($v){
				$new_goodsids_arr[]=$v;
			}
		}

		if(empty($new_goodsids_arr)){
			$rs['code'] = 1001;
			$rs['msg'] = '请选择商品收藏记录';
			return $rs;
		}
		...
		$domain=new Domain_Shop();
		$res=$domain->batchDelCollect($uid,$new_goodsids_arr);
	 ...
	 * @return string msg 提示信息
	 * @return array info 返回信息
	 * @return int info[0].id 返回商品id
	 * @return int info[0].uid 返回商品发布者id
	 * @return string info[0].name 返回商品名称
	 * @return int info[0].sale_nums 返回商品销量
	 * @return int info[0].type 返回商品类型 0 站内商品 1 站外商品 2 平台自营商品
	 * @return float info[0].original_price 返回商品原价
	 * @return int info[0].status 返回商品状态
	 * @return string info[0].thumb 返回商品封面
	 * @return float info[0].price 返回商品现价
	 */
	public function getVideoBindGoodsInfo(){
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
		$goodsid=checkNull($this->goodsid);
		...
		$res=$domain->getVideoBindGoodsInfo($goodsid);
		$rs['info']=$res;
		return $rs;
	}
	...
	/**
	 * 主播设置商品是否直播间内展示
	 * @desc 用于主播设置商品是否直播间内展示
	 * @return int code 操作码，0表示成功
	 * @return array info 
	 * @return string msg 提示信息
	 */
	public function setShowGoods(){
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
		
		$uid=checkNull($this->uid);
		$token=checkNull($this->token);
		$goodsid=checkNull($this->goodsid);
		$isshow=checkNull($this->isshow);
		...
		$domain = new Domain_Shop();
		$res = $domain->setShowGoods($uid,$goodsid,$isshow);
