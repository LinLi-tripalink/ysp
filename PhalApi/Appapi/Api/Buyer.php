...
class Api_Buyer extends PhalApi_Api {
	...
	public function getRules() {
		return array(
			...
			'goodsOrderPay'=>array(
				...
				'type' => array('name' => 'type', 'type' => 'int',  'desc' => '支付类型 1 支付宝 2 微信 3 余额 4 微信小程序 5 paypal 6 braintree_paypal'),
				'time' => array('name' => 'time', 'type' => 'string', 'desc' => '时间戳'),
				'sign' => array('name' => 'sign', 'type' => 'string', 'desc' => '签名字符串'),
			),
    ...
    public function goodsOrderPay(){
        ...
        if($uid<0||$token==""||$orderid<1||!in_array($type, ['1','2','3','6'])||$time<0||!$sign){
        	$rs['code']=1001;
        	$rs['msg']='参数错误';
        	return $rs;
        }
	...
	*/
    ...
    public function evaluateGoodsOrder(){
        ...
        if($thumbs){
            ...
            foreach ($thumb_arr as $k => $v) {
            	$thumbs_format[]=setCloudType($v);
            }

            $thumbs_format_str=implode(',', $thumbs_format);
        }
    ...
    public function applyPlatformInterpose(){
        ...
        $data=array(
            ...
            'platform_interpose_thumb'=>setCloudType($thumb)
        );
