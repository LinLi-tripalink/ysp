...
class Api_Video extends PhalApi_Api {
	...
	public function getRules() {
		return array(
            ...
            'setVideo' => array(
                ...
                'is_userad' => array('name' => 'is_userad', 'type' => 'string', 'default'=>'0','desc' => '是否是用户广告视频0否1是'),
                'userad_url' => array('name' => 'userad_url', 'type' => 'string', 'default'=>'','desc' => '用户广告视频链接'),
	...
	public function setVideo() {
        ...
        $is_userad=checkNull($this->is_userad);
        $userad_url=checkNull($this->userad_url);
    ...
    public function getVideoListByClass(){
        ...
        $domain=new Domain_Video();
        $result=$domain->getVideoListByClass($uid,$classid,$p);
    ...
    public function setVideoPay(){
		...
		//检测青少年模式是否开启
		$checkTeenagers=checkTeenagers($uid);
		if($checkTeenagers){
            $rs['code'] = 1001;
            $rs['msg'] = '青少年模式下不支持该功能';
            return $rs;
    ...
    */
    ...
    public function getCosInfo(){
        ...
        $qiniuInfo=array(
            ...
            'qiniu_zone'=>'qiniu_hb'  //华东:qiniu_hd 华北:qiniu_hb  华南:qiniu_hn  北美:qiniu_bm   新加坡:qiniu_xjp 不可随意更改，app已固定好规则
        );
     ...
     * @return string msg 提示信息
     * @return array info 返回信息
     * @return int info[0].iscollection 是否收藏视频 0 否 1 是
    ...
    public function checkPayVideoIsShare(){
        ...
        $data['iscollection']=(string)isCollection($uid,$videoid);
    ...
    /**
     * 获取用户发布的广告视频列表
     * @desc 用于获取用户发布的广告视频列表
     * @return int code 状态码，0表示成功
     * @return string msg 提示信息
     * @return array info 返回信息
     */
    public function getUserAdVideos(){
        $rs=array('code'=>0,'msg'=>'','info'=>array());
        $uid=checkNull($this->uid);
        $token=checkNull($this->token);
        $p=checkNull($this->p);
        ...
        $domain=new Domain_Video();
        $res=$domain->getUserAdVideos($uid,$p);
     ...
     * @return int code 操作码，0表示成功
     * @return array info 
     * @return string info[0].iscollection 是否收藏 1是  0否
     * @return string info[0].collections 收藏数量
     * @return string msg 提示信息
     */
	public function addCollection() {
        ...
        $domain = new Domain_Video();
        $result = $domain->addCollection($uid,$videoid);
		if($result==1001){
			$rs['code'] = 1001;
			$rs['msg'] = "视频已删除";
			return $rs;
