...
class Model_Shop extends PhalApi_Model_NotORM {
	...
	public function getShop($uid,$fields='') {
        ...
        if($uid==1){ //平台自营店铺
            ...
            $shop_info['name']=$configpub['app_name'].'小店';
        }else{
        ...
        if($uid==1){
            $shop_info['avatar']=get_upload_path("/default.png");
        }else{
    ...
    public function getGoodsList($where,$p){
        ...
        foreach ($list as $k => $v) {
            ...
            if($v['type']==1){ //外链商品
                ...
                $list[$k]['old_price']=$v['original_price']; //ios专用
                $list[$k]['specs']=[];
            }else{
