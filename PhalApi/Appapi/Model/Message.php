...
class Model_Message extends PhalApi_Model_NotORM {
	...
	public function officialLists($p){
		...
		$list=DI()->notorm->admin_push
			->select("id,title,synopsis,type,url,pushtime")
			->where(['is_push'=>1])
			->order("pushtime desc")
			->limit($start,$nums)
			->fetchAll();
		...
		foreach ($list as $k => $v) {
			...
			$list[$k]['addtime']=datetime($v['pushtime']);
	...
	public function getLastTime($uid){
		...
		$officeInfo=DI()->notorm->admin_push->where(['is_push'=>1])->order("pushtime desc")->fetchOne();
		...
		if($officeInfo){
			$officeInfo['addtime']=$officeInfo['pushtime'];
