...
class Model_Live extends PhalApi_Model_NotORM {
	...
	public function checkBan($uid){
        ...
        if($isexist){
			$time=time();
			if($isexist['end_time']==0 || $isexist['end_time']>$time){
				return 1;
			}
	...
	// 超管关闭直播间
	public function superStopRoom($uid,$token,$liveuid,$type,$banlong){
		...
		if($type==1){
			...
			$long_arr=explode('_',$banlong); //将字符串分隔
			$long=$long_arr['0'];
            ...
            DI()->notorm->user_live_ban->insert([ 'uid'=>$liveuid,'superid'=>$uid,'addtime'=>$time,'end_time'=>$long ]);
		}
