...
class Model_Video extends PhalApi_Model_NotORM {
	...
	public function addCommentLike($uid,$commentid){
		...
		if(!$fabulous){
			...
			jMessageIM("点赞通知",$commentinfo['uid'],"dsp_like");
	...
	public function del($uid,$videoid){
		...
		if($result){
			...
			//将用户收藏视频列表状态修改
			DI()->notorm->user_video_collection
				->where("videoid='{$videoid}'")
				->update(array("status"=>0));
	...
	public function getHomeVideo($uid,$touid,$p){
		...
		if($uid==$touid){  //自己的视频（需要返回视频的状态前台显示）
			$where=" uid={$uid} and isdel='0' and status=1  and is_ad=0 and is_userad=0";
		}else{  //访问其他人的主页视频
			...
			$where=" uid={$touid} and isdel='0' and status=1  and is_ad=0 and is_userad=0";
		}
    ...
    //根据视频分类id获取视频列表
    public function getVideoListByClass($uid,$classid,$p){
    	$pnums=50;
    	if($p<1){
    		$p=1;
    	}
	...
	//用户广告视频列表
	public function getUserAdVideos($uid,$p){
		$pnums=50;
		$start=($p-1)*$pnums;

		$list=DI()->notorm->user_video
			->where(['uid'=>$uid,'status'=>1,'isdel'=>0,'is_userad'=>1])
			->order("addtime desc")
			->limit($start,$pnums)
			->fetchAll();
	...
	/* 收藏 */
	public function addCollection($uid,$videoid){
		$rs=array(
			'iscollection'=>'0',
			'collections'=>'0',
		);
		$video=DI()->notorm->user_video
				->select("collections,uid,thumb")
				->where("id = '{$videoid}'")
				->fetchOne();
		...
		if($like){
			...
			DI()->notorm->user_video
				->where("id = '{$videoid}' and collections>0")
		...
		$rs['collections']=NumberFormat($video['collections']);
