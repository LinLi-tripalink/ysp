<?php
	...
	function getUserInfo($uid,$tree='') {
		...
		if($uid==0){
			...
			if(!$info){
				$info=DI()->notorm->user
						->select('id,user_nicename,avatar,avatar_thumb,sex,signature,province,city,area,birthday,age,issuper,coin,votestotal,consumption,user_status,isrecommend,recommend_time,is_firstlogin,bg_img')
	...
	function getWorks($uid) {
		$count=DI()->notorm->user_video
				->where('uid=? and isdel=0 and is_userad=0 and status=1 and is_ad=0',$uid)
				->count();
		return 	$count;
	}
	...
	function getCollections($uid) {
		...
		$count=DI()->notorm->user_video_collection
				->where('uid=? and status=1',$uid)  //status=1表示视频状态正常，未被二次拒绝或被下架
				->count();
	...
	*/
	...
	/* 视频是否收藏 */
	function isCollection($uid,$videoid){
		$like=DI()->notorm->user_video_collection
				->select("id")
				->where("uid='{$uid}' and videoid='{$videoid}'")
				->fetchOne();
		if($like){
    ...
    function handleVideo($uid,$v,$tree){
        ...
        $v['islike']='0';	
        $v['isattent']='0';
        $v['teenagers_switch']='0';
        
		if($uid>0){
			$v['islike']=(string)ifLike($uid,$v['id']);	
			$v['iscollection']=(string)isCollection($uid,$v['id']);	
			$v['teenagers_switch']=(string)checkTeenagers($uid);	
		}
	...
	*/
	...
	function checkVideoIsAd($videoid){
		$info=DI()->notorm->user_video->where("id=? and (is_ad=1 or is_userad=1)",$videoid)->fetchOne();
		if(!$info){
			return 0;
		}
	...
	function checkLiveBan($uid){
        ...
        if($isexist){
			...
			$time=time();
			if($isexist['end_time']==0){
				$arr['score']='1';
				$arr['msg']='您直播间已被永久封禁';
				
				return $arr;
			}else if($isexist['end_time']>$time){
				$arr['score']='1';
				$arr['msg']='您直播间已被封禁，截止日期：'.date('Y年m月d日 h:i',$isexist['end_time']);
